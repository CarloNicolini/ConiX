"""General QCP solver wrapping the ConiX C ABI (Clarabel-compatible form).

Problem form (same as Clarabel / COSMO):
...
class SolverSolution:
    ...
    def z(self) -> np.ndarray:
        """Alias for Clarabel-style dual cone multiplier."""
        return self.y


class ConixSolver:
    """Persistent ConiX workspace for a fixed sparsity pattern."""
    ...
    def warm_start(self, x=None, y=None, s=None, z=None) -> None:
        dual = z if z is not None else y
        xp = sp = zp = None
        if x is not None:
            xv = np.asarray(x, dtype=np.float64).reshape(-1)
            xp = (c_double * self._n)(*[float(v) for v in xv])
        if s is not None:
            sv = np.asarray(s, dtype=np.float64).reshape(-1)
            sp = (c_double * self._m)(*[float(v) for v in sv])
        if dual is not None:
            zv = np.asarray(dual, dtype=np.float64).reshape(-1)
            zp = (c_double * self._m)(*[float(v) for v in zv])
        if self._lib.conix_warm_start(self._ptr, xp, sp, zp) != 0:
            raise RuntimeError(self._cx.last_error())


def solve(P, q, A, b, cones: list, **kwargs) -> SolverSolution:
    """One-shot solve."""
    with ConixSolver(P, q, A, b, cones, **kwargs) as solver:
        return solver.solve()
